...
STRIPE_PRICE_IDS = {
    "starter_monthly": os.getenv(
        "STRIPE_STARTER_MONTHLY_PRICE_ID",
        "price_starter_monthly"),
    "starter_annual": os.getenv(
...
class BillingService:
    ...
    async def record_usage(self,
        ...
        try:
            ...
            logger.info(
                f"Recorded {quantity} {usage_type} usage for org {org_id}")
    ...
    async def _report_stripe_usage(
        ...
        try:
            ...
            if not subscription_info or not subscription_info.get(
                    'stripe_subscription_id'):
                return

            # Report usage to Stripe (for usage-based add-ons)
            # This is a placeholder - in production you'd configure usage-based
            # pricing
            pass
    ...
    def _create_mock_checkout_session(
            self, tier: str, annual: bool) -> Dict[str, Any]:
    ...
    async def _update_organization_subscription(
            self,
            org_id: str,
            tier: str,
            subscription_id: str,
    ...
    async def _get_subscription_info(
            self, org_id: str) -> Optional[Dict[str, Any]]:
...
async def record_simulation_usage(
        org_id: str, metadata: Dict[str, Any] = None) -> bool:
    """Record a simulation usage event."""
    service = get_billing_service()
    return await service.record_usage(org_id, "simulations", 1, metadata)


async def record_optimization_usage(
        org_id: str, metadata: Dict[str, Any] = None) -> bool:
